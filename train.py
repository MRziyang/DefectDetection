...
parser.add_argument('--num_class', default=2, type=int, help='number of class in ur dataset')
parser.add_argument('--dataset_root', default='./data/VOCdevkit',
                    help='Dataset root directory path')
parser.add_argument('--basenet', default='resnet50-19c8e357.pth', type=str, choices=['vgg16_reducedfc.pth', 'efficientnet_b4_truncated.pth','resnet50-19c8e357.pth'],
...
def train():
    ...
    elif args.basenet == 'resnet50-19c8e357.pth':
        ssd_net = build_ssd('train', args.input, args.num_class)
    net = ssd_net
    ...
    if args.cuda:
        ...
        # net = torch.load('weights\92.2Baseline+SeLayer+FDCloss+deformable_module.pth')
        # print("--------------------成功读取")
        net=ssd_net.cuda()
    if args.resume:
    ...
    for epoch in range(args.start_epoch, args.num_epoch):
        ...
        for images, images_origin,targets in data_loader: # load train data 目的是要测试孪生网络下是否能显示突变性模糊映射缺陷分类
            # if iteration % 100 == 0:
            ...
            if args.cuda:
                ...
                images_origin = Variable(images_origin.cuda())
                targets = [Variable(ann.cuda()) for ann in targets]

            else:
                images = Variable(images)
                images_origin = Variable(images_origin)
                targets = [Variable(ann) for ann in targets]
            # forward
            t0 = time.time()
            out,f_img,f_img_origin = net(images,images_origin)
            # backprop
            optimizer.zero_grad()
            loss_l, loss_c,DCloss = criterion(out,f_img,f_img_origin, targets)
            loss = loss_l + loss_c
            loss.backward()
            optimizer.step()
            t1 = time.time()
            ...
            DCloss+=DCloss.item()
            if iteration % 10 == 0:
                print('Epoch '+repr(epoch)+'|| iter ' + repr(iteration % epoch_size)+'/'+repr(epoch_size) +'|| Total iter '+repr(iteration)+ ' || Total Loss: %.4f || Loc Loss: %.4f || Cls Loss: %.4f ||DCloss:%.4f || LR: %f || timer: %.4f sec.\n' % (loss.item(),loss_l.item(),loss_c.item(),DCloss.item(),cur_lr,(t1 - t0)), end=' ')
                loss_cls.append(loss_c.item())
                loss_loc.append(loss_l.item())
                loss_cls_dc.append(DCloss.item())

...
'''Baseline+smooth_module+Selayer+deformable_module'''
...
class SSD(nn.Module):
    ...
    def forward_once(self, x,mode):
        ...
        for k, v in enumerate(self.extras):
            ...
            if k % 2 == 1:
                sources.append(x)
                def_x = DeformConvNet(x.shape[1])(x) #1 可变性卷积
                x = def_x + x
                # if mode == 'ori':  # 对后面无用的分支进行剪枝
                #     break
        # if mode == 'def':
    ...
    def forward(self, x, x_origin):
        x_output, feat_x = self.forward_once(x,mode='def')
        _, feat_x_origin = self.forward_once(x_origin,mode='ori')
...
def Decode_feature(block, x, before_feat, sourceList):
    '''还原回38*38'''  # x=2048,10,10; 2048-->1024-->512

    # x = x + before_feat[2] #一会儿再试试

    for i in range(0, 4):
        x = block[i](x)
    x=x+before_feat[1]
    # se_x = SELayer(x.shape[1])(x) #3 selayer
    # x = x + se_x + before_feat[1]
    x_1024_f = x

    for i in range(4, 9):
        x = block[i](x)
    x = x + before_feat[0]
    # se_x = SELayer(x.shape[1])(x)
    # x = x + se_x + before_feat[0]
    x_512_f = x
